...
const getImageUrl = (image) => {
  if (!image) {
    return "https://placehold.co/180x180/f8fafc/94a3b8?text=ShopEase";
  }
...
const TrackOrderPage = () => {
  ...
  const activeDelivery = useMemo(() => {
    ...
    return (
      delivery.find((entry) => entry.status !== "cancelled") ||
      delivery[0] ||
      null
    );
  }, [order?.delivery]);
    ...
    <main className="min-h-screen bg-[#f7f8fb] text-slate-950">
        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">
              Use your ShopEase order number together with the email address or
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <form
          ...
          className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_24px_80px_-40px_rgba(15,23,42,0.24)] sm:p-7"
        >
          <div className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <div>
              ...
              <input
                ...
                placeholder="ORD-20260815-1234567890"
                autoComplete="off"
                className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-violet-300 focus:bg-white focus:ring-4 focus:ring-violet-50"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={tracking}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-violet-700 disabled:cursor-wait disabled:opacity-60"
            >
              {tracking ? (
                <>
          ...
          <p className="mt-4 text-xs leading-5 text-slate-400">
            For privacy, ShopEase only returns an order when both the order
            number and customer contact information match.
          </p>
        </form>
        ...
        {order && (
          ...
          >
            <section className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_24px_80px_-40px_rgba(15,23,42,0.22)]">
              <div className="flex flex-col gap-4 border-b border-slate-100 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7">
                <div>
                  ...
                  <h2 className="mt-2 font-mono text-base font-bold sm:text-lg">
                    {order.orderNumber}
                  </h2>
                ...
                >
                  ...
                  {String(order.status || "pending").charAt(0).toUpperCase() +
                    String(order.status || "pending").slice(1)}
                </span>
              </div>

              {order.status === "cancelled" ? (
                <div className="m-6 rounded-2xl border border-red-100 bg-red-50 p-5 sm:m-7">
                  <p className="font-bold text-red-700">This order was cancelled.</p>
                  <p className="mt-1 text-sm leading-6 text-red-600/80">
                    If you believe this is unexpected, contact ShopEase and keep
                      ...
                      return (
                        <div key={step.key} className="relative">
                          {index < STATUS_STEPS.length - 1 && (
                            <div
                              className={`absolute left-5 top-5 hidden h-0.5 w-[calc(100%-1rem)] sm:block ${
                                index < currentRank
                                  ? "bg-emerald-400"
                                  : "bg-slate-200"
                              }`}
                          ...
                          <div className="relative z-10 flex items-center gap-3 sm:block">
                            ...
                            <p
                              className={`text-xs font-bold sm:mt-3 ${
            ...
            {activeDelivery && order.status !== "cancelled" && (
              <section className="rounded-[30px] border border-blue-100 bg-gradient-to-br from-blue-50 to-violet-50 p-6 sm:p-7">
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="rounded-2xl bg-white/80 p-4">
                    ...
                    <p className="mt-2 font-bold text-slate-950">
                      {activeDelivery.courierName || "Not assigned yet"}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-white/80 p-4">
                    <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">
                    ...
                    <p className="mt-2 break-all font-mono text-sm font-bold text-slate-950">
                      {activeDelivery.trackingId || "Not available yet"}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-white/80 p-4">
                    <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">
                    ...
                    <p className="mt-2 text-sm font-bold text-slate-950">
                      {formatDateTime(activeDelivery.lastUpdatedAt)}
                    </p>
                  </div>
                </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-600">
                          {activeDelivery.riderName || "Assigned rider"}
                        </p>
                        {activeDelivery.riderPhone && (
                          <a
                            href={`tel:${activeDelivery.riderPhone}`}
                            className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-emerald-700"
                          >
              ...
              <div className="space-y-6">
                <div className="rounded-[28px] bg-slate-950 p-6 text-white">
                  ...
                  <div className="mt-5 space-y-3 text-sm">
                    ...
                    <div className="flex justify-between gap-4 border-t border-white/10 pt-4 text-base font-black">
                      <span>Total</span>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link
                to="/products"
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3.5 text-sm font-bold text-white transition hover:bg-slate-800"
              >
                <FaShoppingBag /> Continue shopping
              </Link>
              <Link
                to="/contact"
                className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 py-3.5 text-sm font-bold text-slate-700 transition hover:bg-slate-50"
              >
